...
const avif = require(__htmldev_root + '/lib/module/processing/avif.js');

const readDirFiles = require(__htmldev_root + '/lib/module/files/readDirFiles.js');
...
async function processFile(file, options) {
	switch (file.type) {
		...
		case 'png':
			...
			await avif(file.FILE_PATH, file.NO_FILE_TYPE_DIST_PATH + ".avif");
...
module.exports = async function (options = {}) {
	...
	let LAST_BUILD;
	try { LAST_BUILD = JSON.parse(fs.readFileSync('.htmldev/dist/build_export.json')); }
	...
	for (const FILE_KEY in files) {
		...
		if (isNotInLastBuild(LAST_BUILD, file, FILE_KEY)) {
			...
			file.NO_FILE_TYPE_DIST_PATH = process.cwd() + ".htmldev/dist/export" + FILE_KEY.substring(0, FILE_KEY.lastIndexOf('.')); // The path of the file with no file ending ( 'test.js' = 'test' )
			file.DIST_FOLDER = process.cwd() + ".htmldev/dist/export" + FILE_KEY.substring(0, FILE_KEY.lastIndexOf("/")); // The directory path of the target file

			// Ensure the output directory
			fs.ensureDirSync(file.DIST_FOLDER);

			colorLog(['yellow', 'FILES :: Building '],
				['cyan', `/export${FILE_KEY}`]);

			CHANGED_COUNT++;

			await processFile(file, options);

			if(FILE_KEY === '/img/icons/facebook.png') delete files[FILE_KEY];
		}
	}

	fs.writeFileSync('.htmldev/dist/build_export.json', JSON.stringify(files));
