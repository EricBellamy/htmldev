...
const lazyload = require(__htmldev_root + '/lib/module/lazyload.js');
...
module.exports = async function (parsed, RELATIVE_FILE_PATH) {
	let LAZY_URLS = [];
	...
	while (includeNodes.length != 0) {
				...
				if (ext.indexOf('.') === -1) {
					...
					switch (ext) {
						...
						case '.svg':
							...
							const processErrors = await includeLinks[ext].process(parsed, node, contents, LAZY_URLS, `include/${noDotExt}/${node.attributes.src}`, RELATIVE_FILE_PATH);
		...
		includeNodes = parsed.querySelectorAll('include');
	}

	// Remove duplicate lazy URLs
	LAZY_URLS = [...new Set(LAZY_URLS)];

	parsed.querySelector('head').innerHTML = await lazyload.pre() + parsed.querySelector('head').innerHTML;
	parsed.querySelector('body').innerHTML += await lazyload.post(LAZY_URLS);
	// parsed.querySelector('body').innerHTML += `<script>console.log("HERE?");</script>`;
}