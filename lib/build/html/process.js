...
module.exports = {
	load: async function (FILE_PATH, INPUT_DATA) {
		...
		await global.HTMLDEV_PRIVATE.activateHook("process", "start", parsed, RELATIVE_FILE_PATH);
		...
		if (validation === true) {
			...
			await global.HTMLDEV_PRIVATE.activateHook("process", "finish", parsed, RELATIVE_FILE_PATH);
