...
module.exports = async function () {
	...
	let LAST_BUILD;
	try { LAST_BUILD = JSON.parse(fs.readFileSync('.htmldev/build_html.json')); }
	...
	for (const FILE_KEY in pageFiles) {
		...
		if (file.type === 'html' && isNotInLastBuild(LAST_BUILD, file, FILE_KEY)) {
			...
			fs.writeFileSync(`.htmldev/dist/${FILE_KEY_PARTS[1]}`, LOADED_FILE.html);
	...
	fs.writeFileSync('.htmldev/build_html.json', JSON.stringify(files));
